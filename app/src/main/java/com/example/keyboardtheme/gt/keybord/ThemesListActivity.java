...
public class ThemesListActivity extends Activity implements OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
                            long id) {
        // TODO Auto-generated method stub
        if (AppConstants.getTheme(ThemesListActivity.this).get(position).isBlock()) {
            if (preference.getValueCoin() >= 2) {
                preference.setValueCoin(preference.getValueCoin() - 2);
                Set<String> keyList = preference.getListKeyBy();
                keyList.add(AppConstants.getTheme(ThemesListActivity.this).get(position+1).getTheme()+"");
                preference.setListKeyBy(keyList);
                listAdapter.listPositionBlock = AppConstants.getTheme(ThemesListActivity.this);
                ...
                alertDialog.setTitle("You don't have enough gold ")
                        ...
                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                Intent intent = new Intent(ThemesListActivity.this, PurchaseInAppActivity.class);
                                ThemesListActivity.this.startActivity(intent);
                            }
                        });
